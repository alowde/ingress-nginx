...
var _ = framework.DescribeSetting("[Load Balancer] Least Connections", func() {
	...
	ginkgo.BeforeEach(func() {
		f.NewEchoDeployment(framework.WithDeploymentReplicas(1), framework.WithServiceName("leastconn-slow"))
		f.NewAlwaysSlowEchoDeploymentWithOptions(1800, framework.WithDeploymentReplicas(1), framework.WithServiceName("leastconn-slow"))
	...
	ginkgo.It("sends fewer requests to a slower server", func() {
		...
		assert.Equal(ginkgo.GinkgoT(), replicaCount, 2, "checking expected number of replicas responded")
		values := make([]int, 2)
		i := 0
		for _, v := range replicaRequestCount {
			values[i] = v
		...
		assert.GreaterOrEqual(ginkgo.GinkgoT(), values[1], 2*values[0])
	})
})
